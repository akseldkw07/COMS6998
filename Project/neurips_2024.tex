...
\begin{equation}
		\mathcal{L} = \mathrm{CE}(\text{current}) + \alpha \cdot \mathrm{MSE}(\text{replay logits}) + \beta \cdot \mathrm{CE}(\text{replay labels})
\end{equation}
...
\begin{equation}
		d_i = \max\!\left(1 - \mathrm{cos\_sim}\!\left(\mathrm{embed}(T_i),\, \mathrm{embed}(T_{\text{new}})\right),\; 0.01\right)
\end{equation}
...
The DER++ and similarity buffer analysis had the following limitations. Tasks are still learned sequentially without revisiting (once a dataset is visited in the 8-day curriculum, it is not seen
...
\begin{thebibliography}{00}
		...
		\bibitem{mir}
		Aljundi, Rahaf, Eugene Belilovsky, Tinne Tuytelaars, Laurent Charlin, Massimo Caccia, Min Lin, and Lucas Page-Caccia. "Online continual learning with maximal interfered retrieval." Advances in neural information processing systems 32 (2019).

		\bibitem{gss}
		Aljundi, Rahaf, Min Lin, Baptiste Goujaud, and Yoshua Bengio. "Gradient based sample selection for online continual learning." Advances in neural information processing systems 32 (2019).
		...
		Liang, Victor Weixin, Yuhui Zhang, Yongchan Kwon, Serena Yeung, and James Y. Zou. "Mind the gap: Understanding the modality gap in multi-modal contrastive representation learning." Advances in Neural Information Processing Systems 35 (2022): 17612-17625.
